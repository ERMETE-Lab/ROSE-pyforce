...
def plot_geim_sensors(  geim: GEIM, m_sens_to_plot: int, tex_var_name: str,
                        cmap = None, resolution = [800, 800]):
    ...
    sens_to_plot = Function(geim.magic_sens.fun_space)
    
    for mm in range(m_sens_to_plot):
        sens_to_plot.x.array[:] += geim.magic_sens(mm) / max(geim.magic_sens(mm))
    
    plotter.subplot(0,0)
    clim = [0.,  max(sens_to_plot.x.array)]
    dict_cb['title'] = r'Sensors - $'+tex_var_name+'$'
    plotter.add_mesh(grids(sens_to_plot)[0], cmap = map1, clim = clim, show_edges=False, scalar_bar_args=dict_cb)    
    
    sgreedy_sens = sens_pos(geim.magic_sens, geim.V.mesh, m_sens_to_plot)
    ...
    plotter.add_text(r'$s = {:.2f}'.format(geim.sens_class.s)+'$',
                        color= 'k', position='upper_edge', font_size=25)
    
    plotter.add_text(r'Sensors for $'+tex_var_name+'$',
